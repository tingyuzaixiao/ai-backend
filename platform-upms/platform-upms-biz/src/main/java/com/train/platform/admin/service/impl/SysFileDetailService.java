...
import com.train.platform.admin.api.entity.SysFileDetail;
import com.train.platform.admin.mapper.SysFileDetailMapper;
 ...
 */
@Service
public class SysFileDetailService extends ServiceImpl<SysFileDetailMapper, SysFileDetail> implements FileRecorder {
	...
	public boolean save(FileInfo info) {
		...
		userId =  userId.length() < 4 ? "000" + userId : userId;
		long infoId = Long.parseLong(TimeId.nextPkId() + userId.substring(userId.length() - 4));
		info.setId(Long.toString(infoId));
	...
	public void update(FileInfo info) {
		SysFileDetail detail = toFileDetail(info);
		QueryWrapper<SysFileDetail> qw = new QueryWrapper<SysFileDetail>()
			.eq(detail.getUrl() != null, SysFileDetail.COL_URL, detail.getUrl())
			.eq(detail.getId() != null, SysFileDetail.COL_ID, detail.getId());
		update(detail, qw);
	}
	...
	public FileInfo getByUrl(String url) {
		return toFileInfo(getOne(new QueryWrapper<SysFileDetail>().eq(SysFileDetail.COL_URL, url)));
	}
	...
	public boolean delete(String url) {
		remove(new QueryWrapper<SysFileDetail>().eq(SysFileDetail.COL_URL, url));
		return true;
	}
	...
	public SysFileDetail toFileDetail(FileInfo info) throws JsonProcessingException {
		SysFileDetail detail = BeanUtil.copyProperties(info, SysFileDetail.class, "metadata", "userMetadata",
