...
import com.train.platform.admin.api.entity.SysFilePartDetail;
import com.train.platform.admin.mapper.SysFilePartDetailMapper;
import lombok.SneakyThrows;
import org.dromara.x.file.storage.core.upload.FilePartInfo;
import org.springframework.stereotype.Service;

/**
 * 用来将文件分片上传记录保存到数据库，这里使用了 MyBatis-Plus 和 Hutool 工具类 目前仅手动分片分片上传时使用
 */
@Service
public class SysFilePartDetailService extends ServiceImpl<SysFilePartDetailMapper, SysFilePartDetail> {
	...
	public void deleteFilePartByUploadId(String uploadId) {
		remove(new QueryWrapper<SysFilePartDetail>().eq(SysFilePartDetail.COL_UPLOAD_ID, uploadId));
	}
