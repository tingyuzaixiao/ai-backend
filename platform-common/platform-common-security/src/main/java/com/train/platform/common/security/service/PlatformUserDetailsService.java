 ...
 * @date 2024/12/21
 */
public interface PlatformUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new PlatformUser(user.getUserId(), user.getDeptId(), user.getUsername(),
	 ...
	 * @return
	 */
	default UserDetails loadUserByUser(PlatformUser platformUser) {
		return this.loadUserByUsername(platformUser.getUsername());
	}
